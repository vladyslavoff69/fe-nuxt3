...
export default class WHEPClient {
  ...
  constructor(endpoint: string, videoElement: HTMLVideoElement | undefined) {
    ...
    this.peerConnection.ontrack = (event: RTCTrackEvent) => {
      ...
      const currentTracks: MediaStreamTrack[] = this.stream.getTracks()
      const streamAlreadyHasVideoTrack: boolean = currentTracks.some(track => track.kind === 'video')
      const streamAlreadyHasAudioTrack: boolean = currentTracks.some(track => track.kind === 'audio')
