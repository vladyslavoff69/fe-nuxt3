...
<template>
  <div class="flex items-center">
    ...
    <div class="flex-1 overflow-hidden relative px-1 psuedo-gradient">
      <JScrollBar
        v-if="gamesLoading || loading"
        :has-auto-scrolling="true"
      >
        <div class="flex items-center">
          <SkeletonWageredGame
            v-for="i in 10"
      ...
      >
        <div class="flex items-center">
          <div
            v-for="(highRoller, idx) in wageredGames"
            :key="`key-${idx}-hey-a-2`"
            class="border-l border-l-border px-3"
          >
            <h5 class="text-lg sm:text-xl text-white font-bold">
              ${{ highRoller.amount ? numberWithCommas(highRoller.amount, 2) : 0 }}
            </h5>
            <div class="flex items-center gap-x-0.5">
              <template v-if="GAME_MATH_ICONS[highRoller.game.slug]?.type === 'ui'">
                <UIcon
                  :name="GAME_MATH_ICONS[highRoller.game.slug]?.icon || ''"
                  class="w-2 h-2 min-w-2 min-h-2"
                />
              </template>
              <template v-else-if="GAME_MATH_ICONS[highRoller.game.slug]?.icon">
                <svg
                  v-if="GAME_MATH_ICONS[highRoller.game.slug]?.icon"
                  class="w-2 h-2 min-w-2 min-h-2"
                >
                  <use :href="GAME_MATH_ICONS[highRoller.game.slug]?.icon" />
                </svg>
              </template>
              <template v-else>
                <svg
              ...
              <span class="font-bold text-xxs text-nowrap leading-1">{{ highRoller.game.title }}</span>
            </div>
          </div>
        </div>
      </JScrollBar>
...
@media (min-width: 640px) {
  .psuedo-gradient {
    ...
    &::before {
      left: 0;
      background: linear-gradient(88deg, #021802 3.04%, transparent 84.37%);
    }

    &::after {
      right: 0;
      background: linear-gradient(260deg, #051B05 7.3%, rgba(5, 26, 5, 0.64) 46.82%, rgba(5, 27, 5, 0.00) 87.35%);
    }
  }
}
</style>
