...
import { AUTO_BET, KENO_GAME } from '~/apps/keno/constants'
...
const { risk, onOpeningDone, selectedTiles, betResult, inAutoPick, invalid } = inject(KENO_GAME)!
const {
  betCryptoAmount,
...
const { off } = onBetGameDone(({ data: { kenoBetGame } }: any) => {
  // check if user clicked Stop button
  if (playStatus.value !== PLAYING) {
    return
  }

  const { payoutMultiplier } = kenoBetGame
  if (playStatus.value === PLAYING) {
...
<template>
  ...
  >
    ...
    <AutoBetButton
      :invalid="invalid || !selectedTiles.length"
