...
import { AUTO_BET, WHEEL_GAME } from '~/apps/wheel/constants'
...
const { off } = onBetGameDone(({ data: { wheelBetGame } }: any) => {
  // check if user clicked Stop button
  if (playStatus.value !== PLAYING) {
    return
  }

  const { payoutMultiplier } = wheelBetGame
  if (playStatus.value === PLAYING) {
