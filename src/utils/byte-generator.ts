import { getSha256HmacsForGameSeedStartingFromOffset } from './sha256'

import type { GameSeed, InternalGameEventBytes } from '~/types/game'
...
export function getBytesForGameSeedStartingFromOffset (
  ...
  const numberOfHmacs = Math.ceil(
    (numberOfBytes + (numberOfIgnoredBytes % 32)) / 32
  )

  numberOfIgnoredBytes -= numberOfIgnoredHmacs * 32

  const hmacs = getSha256HmacsForGameSeedStartingFromOffset(
    gameSeed,
    numberOfHmacs,
    numberOfIgnoredHmacs
  )
  const bytes = getBytesFromHmacsStartingFromOffsetForFirstHmac(
    numberOfBytes,
    hmacs,
    numberOfIgnoredBytes
  )
